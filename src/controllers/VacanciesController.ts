...
export const createVacancy = async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error) {
      return res
        .status(500)
        .json({ error: "Erro interno ao criar a vaga: " + error.message });
    }

    return res
      .status(500)
      .json({ error: "Erro desconhecido ao criar a vaga." });
  }
};
...
export const getVacancyById = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(500).json({ error: "Erro interno ao buscar a vaga." });
  }
};
...
export const destroyVacancy = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(500).json({ error: "Erro interno ao deletar a vaga." });
  }
};
